...
export const chatearConTamalin = async (req, res) => {
  try {
    ...
    // Si el administrador no manda sesion, creamos un identificador temporal
...
// 2. Obtener cuentas desde tu tabla 'detalle_pedido' en Supabase
    const { data: detalle_pedido, error: errorDetalle } = await supabase
      .from("detalle_pedido")
    ...
    if (!detalle_pedido || detalle_pedido.length === 0) {
      return res.status(200).json({
        respuesta: "¡Hola! En este momento no tenemos la carta registrada."
      });
    }
    // 4. Armar cuentas para la IA
    const cuentasTexto = detalle_pedido.map(p =>
      `- **${p.nombre}**: $${Number(p.precio_unitario).toLocaleString("es-CO")} COP | Cantidad: ${p.cantidad} | Subtotal: $${Number(p.subtotal).toLocaleString("es-CO")} COP`
    ).join("\n");

     // 5. Armar catalogo para la IA
...
2. Da recomendaciones sobre las finansas del negocio y mejora para los productos cuando te lo pidan.
3. Especifica los valores siempre en pesos colombianos ($ COP).
4. Se conciso y completa tus oraciones.
5. solo responde a preguntas relacionadas con el restaurante, sus finansas y sus productos, no respondas a preguntas de otro tipo.
`;
